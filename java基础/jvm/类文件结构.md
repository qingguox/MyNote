...
```java
    ...
    cp_info        constant_pool[constant_pool_count-1];//常量池   是一个数组 存储是常量池项
```

紧接着主次版本号之后的是常量池，常量池的数量是 constant_pool_count-1（**常量池容量计数器是从1开始计数的，将第0项常量空出来是有特殊考虑的，索引值为0代表“不引用任何一个常量池项”**）。
